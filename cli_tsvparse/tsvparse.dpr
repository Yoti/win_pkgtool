...
begin
  ...
  OutString:='';
  OutString:=ExtractSubString(InString, 'http', Chr($09));
  ...
  OutString:='';
  OutString:=ExtractSubString(InString, 'KO5i', Chr($09));
  if OutString <> '' then begin
    if ParamStr(2) <> 'quite'
    then WriteLn('RIF: ' + OutString);
    AssignFile(OutFile, ChangeFileExt(ParamStr(1), '.ZRF'));
    Rewrite(OutFile);
    Write(OutFile, OutString);
    CloseFile(OutFile);
  end;
